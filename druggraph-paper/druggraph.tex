...
1,455,278 interaction edges), we encode each drug as a 1024-bit Morgan fingerprint
...
AUC-ROC of \textbf{0.9933} and Average Precision of \textbf{0.9884}, outperforming
published baselines including DeepDDI (0.920), SSI-DDI (0.961), and MHCADDI (0.974).
...
state-of-the-art AUC of 0.9933, surpassing SSI-DDI~\cite{nyamabo2021} and
...
similarity~\cite{rogers2010} or biological target overlap. These approaches fail
to capture higher-order relational patterns.
...
achieving AUC 0.961 on DrugBank.
...
interpretable DDI prediction. Comprehensive benchmarks by Ren et al.~\cite{ren2023}
confirm that GNN-based link prediction consistently outperforms pairwise classifiers.
Our work adopts the GraphSAGE inductive framework~\cite{hamilton2017}, which
handles large sparse graphs efficiently and generalizes to newly added nodes ---
a property essential for integrating newly approved drugs.
...
a deterministic random feature vector as fallback. The graph is highly dense
(mean degree $\mu = 146.7$, max degree 2,637 for Clozapine), which is consistent
with known scale-free properties of biological interaction networks.
...
(80/10/10) using stratified random link splitting with an equal number of negative
edges sampled for the validation and test sets.
...
of \textbf{0.9884}, surpassing all prior work. The classification report on the
test set (Table~\ref{tab:classification}) shows balanced precision and recall.

\begin{table}[H]
\centering
\small
\caption{AUC-ROC comparison on DrugBank. $\dagger$ results from original papers.}
\label{tab:baselines}
\begin{tabular}{lcc}
\toprule
...
that achieves state-of-the-art AUC-ROC of 0.9933 on DrugBank v5.1.15. By modelling
...
\bibitem{magro2012}
L.~Magro et al., ``Epidemiology and characteristics of adverse drug reactions
caused by drug-drug interactions in an emergency department,''
\textit{Eur. J. Clin. Pharmacol.}, vol.~68, pp.~501--509, 2012.
...
\bibitem{peng2022}
J.~Peng et al., ``Prediction of drug-drug interaction events using graph
neural networks based feature extraction,''
